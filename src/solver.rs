...
fn solver_layer_evaluation_logic(
    ...
    let start: Instant = Instant::now();
    let real_score = new_evaluate(&freq_list, &new_path, threshold - add.unwrap_or(0.0)).0;
    ...
    if under_threshold {
        let scorecast_score = Scorecast::calculate_scorecast_score(iter.current_node.read().score, real_score);
        scorecast.read().update_scorecast_score(iter.path.len() as u8, *child, scorecast_score.0);
